...
export default async function AdminHousesPage() {
  ...
  const [houses, landlords] = await Promise.all([
  ...
  return (
    <DashboardShell estateName={estate?.name ?? ""} role="ADMIN">
      <h1 className="text-2xl font-semibold text-slate-900">Houses</h1>
      <p className="mt-1 max-w-2xl text-slate-600">All houses across the estate.</p>
          ...
          <tbody className="divide-y divide-slate-100">
            {houses.map((house) => (
              <tr key={house.id}>
                ...
                <td className="px-4 py-3 text-slate-600">{house.landlord.fullName}</td>
      ...
      <div className="mt-8 rounded-lg border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3">
          <AddHouseForm
            landlords={landlords.map((l) => ({ id: l.id, fullName: l.fullName }))}
          />
        </div>
      </div>
    </DashboardShell>
