...
export function AddTenantForm({
  ...
  houses: { id: string; houseNumber: string; landlordName: string }[];
    ...
    <div>
          ...
          >
            ...
            {houses.map((house) => (
              <option key={house.id} value={house.id}>
                {house.houseNumber} ({house.landlordName})
              </option>
            ))}
          </select>
        </div>
      ...
      {disabled && (
        <p className="mt-2 text-sm text-slate-500">
          You need at least one house and one living space type before adding a tenant.
        </p>
      )}
      ...
      {state.tenantCode && (
        <p className="mt-2 rounded-md bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
          ...
          share this with them along with their House Code.
        </p>
      )}
    </div>
  );
